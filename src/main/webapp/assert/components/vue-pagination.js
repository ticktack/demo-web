...
(function (Vue) {
    ...
    Vue.component('pagination', {
        ...
        template: '<nav v-show="totalPages > 1"><ul class="pagination"><li><span>第 {{pageNo}}/{{totalPages}} 页</span></li><li v-if="!noPrevious"><a style="cursor:pointer;"v-on:click="selectPage(pageNo - 1)"aria-label="Previous"><span aria-hidden="true">上一页</span></a></li><li v-for="page in pages"v-bind:class="{ \'active\': page.number == pageNo, \'disabled\': page.disabled }"><a style="cursor:pointer;"v-on:click="selectPage(page.number)"v-text="page.text"></a></li><li v-if="!noNext"><a style="cursor:pointer;" v-on:click="selectPage(pageNo + 1)"aria-label="Next"><span aria-hidden="true">下一页</span></a></li><li><span>共 {{totalResult}} 条</span></li></ul></nav>',
        replace: true,
        inherit: false,
        computed: {
            noPrevious: function () {
